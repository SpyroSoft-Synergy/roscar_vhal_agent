...
class Ros2AndroidVhalService(Node):
    ...
    def set_vhal_property_callback(self, request, response):
        if request.prop != 290459441:
            self.get_logger().info('Set property: %d \n' % (request.prop))

            # HVAC_FAN_SPEED
            if request.prop == 0x15400500:
                handle_fan_speed(request)

        response.result = True
        return response

    def handle_fan_speed(self, request):
        if not request.int32_values.empty():
...
def main():
    try:  
        ...
        steps = int(input('Enter the number of steps to change the resistance (positive or negative): '))
        x9c103 = X9C103(cs_chip='gpiochip0', cs_offset=43, inc_chip='gpiochip0', inc_offset=42, ud_chip='gpiochip0', ud_offset=55)
        x9c103.set_resistance(steps)

        service = Ros2AndroidVhalService(x9c103)
